...
{  
	char   Number_Buffer[15];				// itoa buffer (saves flash vs sprintf)
	int pos = 13;
	uint32_t abs_value;
	Number_Buffer[14] = 0;
	if (number < 0) { abs_value = (uint32_t)(-number); } else { abs_value = (uint32_t)number; }
	if (abs_value == 0U) {
		Number_Buffer[pos--] = 0x30;
	} else {
		while (abs_value > 0U && pos >= 0) {
			Number_Buffer[pos--] = (char)(0x30 + (abs_value % 10U));
			abs_value /= 10U;
		}
	}
	if (number < 0 && pos >= 0) { Number_Buffer[pos--] = 0x2D; }
	if (LCD.ShowNum_Mode == Fill_Zero) {
		while ((int32_t)(13 - pos) < len && pos >= 0) { Number_Buffer[pos--] = 0x30; }
	}
	LCD_DisplayString( x, y, &Number_Buffer[pos + 1]);
}
