...
static void _usbh_rndis_recv_data_cb(usbh_cdc_port_handle_t cdc_port_handle, void *arg)
{
    ...
    while (1) {
        if (rndis->rx_pending_data_len == 0) {
            ...
            if (pmsg.DataLength == 0 ||
                    pmsg.DataLength > rndis->max_transfer_size) {
                ESP_LOGE(TAG, "Invalid RNDIS payload length: %lu (max transfer: %lu)",
                         pmsg.DataLength, rndis->max_transfer_size);
                usbh_cdc_flush_rx_buffer(cdc_port_handle);
                return;
            }
        ...
        if (rx_length >= pmsg.DataLength) { // if the ring buffer has enough data for the full packet
            /* RX frames do not need DMA-capable memory: usbh_cdc_read_bytes()
             * copies them out of the CDC ring buffer.  Prefer PSRAM here so a
             * short burst of Wi-Fi/LwIP internal allocations cannot starve the
             * RNDIS receive path.  esp-netif may release this buffer with the
             * normal free(), which is supported for capability allocations. */
            uint8_t *data = heap_caps_malloc_prefer(
                pmsg.DataLength, 2,
                MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT,
                MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
            if (data == NULL) {
                /* The complete payload is already buffered.  Drain exactly
                 * this frame so its bytes cannot be mistaken for the next
                 * RNDIS header and later frames remain recoverable. */
                uint8_t discard[128];
                size_t remaining = pmsg.DataLength;
                ESP_LOGE(TAG, "Failed to allocate %lu-byte RNDIS RX buffer; dropping one frame",
                         pmsg.DataLength);
                while (remaining > 0) {
                    size_t chunk = remaining < sizeof(discard) ? remaining : sizeof(discard);
                    ret = usbh_cdc_read_bytes(cdc_port_handle, discard, &chunk,
                                              pdMS_TO_TICKS(10));
                    if (ret != ESP_OK || chunk == 0) {
                        ESP_LOGE(TAG, "Failed to drain dropped RNDIS frame (%s)",
                                 esp_err_to_name(ret));
                        rndis->rx_pending_data_len = 0;
                        usbh_cdc_flush_rx_buffer(cdc_port_handle);
                        return;
...
static void _usbh_rndis_task(void *arg)
{
    ...
    while (1) {
        ...
        if (events & RNDIS_CONNECTED) {
            if (rndis->config.connect_ready_cb != NULL) {
                ESP_LOGI(TAG, "Waiting for modem data session before RNDIS control initialization");
                int wait_count = 0;
                while (!rndis->config.connect_ready_cb(rndis->config.connect_ready_ctx) && wait_count++ < 1200) {
                    vTaskDelay(pdMS_TO_TICKS(100));
                }
                if (wait_count >= 1200) {
                    ESP_LOGE(TAG, "Timed out waiting for modem data session");
                } else {
                    ESP_LOGI(TAG, "Modem data session ready, continuing RNDIS initialization");
                }
            }
