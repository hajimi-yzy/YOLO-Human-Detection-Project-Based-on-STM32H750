...
#define CAMERA_BUFFER_FRAME_SIZE FRAMESIZE_FHD
#define CAMERA_BUFFER_FRAME_SIZE_NAME "FHD 1920x1080"
#define MAX_JPEG_SIZE           (512U * 1024U)
...
static SemaphoreHandle_t s_camera_control_lock;
...
static volatile int s_runtime_fps_override;
static volatile framesize_t s_runtime_frame_size = CAMERA_FRAME_SIZE;
static framesize_t s_buffer_frame_size = CAMERA_FRAME_SIZE;
static volatile int s_local_stream_socket = -1;
static httpd_handle_t s_local_httpd;

static bool supported_runtime_fps(int fps)
{
    return fps == 5 || fps == 8 || fps == 15 || fps == 20 || fps == 30;
}
...
typedef struct {
    ...
    int width;
    int height;
} runtime_resolution_t;

static const runtime_resolution_t s_runtime_resolutions[] = {
    {.name = "640x480", .frame_size = FRAMESIZE_VGA, .width = 640, .height = 480},
    {.name = "1280x720", .frame_size = FRAMESIZE_HD, .width = 1280, .height = 720},
    {.name = "1920x1080", .frame_size = FRAMESIZE_FHD, .width = 1920, .height = 1080},
};

static const runtime_resolution_t *runtime_resolution_by_name(const char *name)
{
    if (name == NULL) {
        return NULL;
    }
    for (size_t index = 0;
            index < sizeof(s_runtime_resolutions) / sizeof(s_runtime_resolutions[0]);
            ++index) {
        if (strcmp(name, s_runtime_resolutions[index].name) == 0) {
            return &s_runtime_resolutions[index];
        }
    }
    return NULL;
}

static const runtime_resolution_t *runtime_resolution_by_frame_size(
    framesize_t frame_size)
{
    for (size_t index = 0;
            index < sizeof(s_runtime_resolutions) / sizeof(s_runtime_resolutions[0]);
            ++index) {
        if (frame_size == s_runtime_resolutions[index].frame_size) {
            return &s_runtime_resolutions[index];
        }
    }
    return NULL;
...
{
    if (!supported_runtime_fps(fps)) {
        return ESP_ERR_INVALID_ARG;
    }
    s_runtime_fps_override = fps;
    ESP_LOGI(TAG, "Cloud video runtime target changed to %d FPS", fps);
    return ESP_OK;
}

int camera_stream_get_target_fps(void)
{
    const int override = s_runtime_fps_override;
    return override > 0 ? override : CONFIG_VIDEO_UDP_FPS;
}

const char *camera_stream_get_resolution(void)
{
    const runtime_resolution_t *resolution =
        runtime_resolution_by_frame_size(s_runtime_frame_size);
    return resolution == NULL ? "640x480" : resolution->name;
}
...
int camera_stream_get_width(void)
{
    const runtime_resolution_t *resolution =
        runtime_resolution_by_frame_size(s_runtime_frame_size);
    return resolution == NULL ? 640 : resolution->width;
}
...
int camera_stream_get_height(void)
{
    const runtime_resolution_t *resolution =
        runtime_resolution_by_frame_size(s_runtime_frame_size);
    return resolution == NULL ? 480 : resolution->height;
}

static esp_err_t camera_stream_set_resolution(const char *name)
{
    const runtime_resolution_t *requested = runtime_resolution_by_name(name);
    if (requested == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
    if (!s_camera_initialized || s_camera_control_lock == NULL) {
        return ESP_ERR_INVALID_STATE;
    }
    if (requested->frame_size > s_buffer_frame_size) {
        ESP_LOGE(TAG, "Resolution %s exceeds initialized JPEG buffer capacity",
                 requested->name);
        return ESP_ERR_NOT_SUPPORTED;
    }
    if (requested->frame_size == s_runtime_frame_size) {
        return ESP_OK;
    }
    if (xSemaphoreTake(s_camera_control_lock, pdMS_TO_TICKS(3000)) != pdTRUE) {
        return ESP_ERR_TIMEOUT;
    }

    sensor_t *sensor = esp_camera_sensor_get();
    const framesize_t previous = s_runtime_frame_size;
    esp_err_t result = ESP_ERR_INVALID_STATE;
    if (sensor != NULL && sensor->set_framesize != NULL &&
            sensor->set_framesize(sensor, requested->frame_size) == 0) {
        s_runtime_frame_size = requested->frame_size;
        result = ESP_OK;
    } else if (sensor != NULL && sensor->set_framesize != NULL) {
        /* Keep the previously confirmed output if the new sensor mode fails. */
        sensor->set_framesize(sensor, previous);
        result = ESP_FAIL;
    }
    xSemaphoreGive(s_camera_control_lock);

    if (result == ESP_OK) {
        if (s_local_frame_lock != NULL &&
                xSemaphoreTake(s_local_frame_lock, pdMS_TO_TICKS(20)) == pdTRUE) {
            s_local_frame_valid = false;
            xSemaphoreGive(s_local_frame_lock);
        }
        ESP_LOGI(TAG, "Camera output resolution changed to %s", requested->name);
    } else {
        ESP_LOGE(TAG, "Camera rejected output resolution %s", requested->name);
    }
    return result;
}
...
esp_err_t camera_stream_set_video_settings(int fps, const char *resolution)
{
    ...
    if (resolution != NULL) {
        ESP_RETURN_ON_ERROR(camera_stream_set_resolution(resolution), TAG,
                            "Failed to change camera resolution");
    }
    return camera_stream_set_target_fps(fps);
}
...
static esp_err_t camera_init(void)
{
    ...
    const framesize_t buffer_frame_size =
        has_psram ? CAMERA_BUFFER_FRAME_SIZE : CAMERA_FRAME_SIZE;
    const char *buffer_frame_size_name =
        has_psram ? CAMERA_BUFFER_FRAME_SIZE_NAME : CAMERA_FRAME_SIZE_NAME;
    ...
    camera_config_t config = {
        ...
        /* Allocate JPEG DMA/frame buffers for the largest runtime option.
         * The sensor output is restored to the configured default below. */
        .frame_size = buffer_frame_size,
    ...
    ESP_LOGI(TAG,
             "Initializing OV5640 JPEG buffers for %s, default output %s "
             "(quality=%d, PSRAM=%s, fb=%u, grab=%s)",
             buffer_frame_size_name, CAMERA_FRAME_SIZE_NAME,
             CAMERA_JPEG_QUALITY,
    ...
    if (sensor == NULL || sensor->set_framesize == NULL ||
            sensor->set_framesize(sensor, CAMERA_FRAME_SIZE) != 0) {
        ESP_LOGE(TAG, "Failed to restore default camera output %s",
                 CAMERA_FRAME_SIZE_NAME);
        esp_camera_deinit();
        return ESP_FAIL;
    }
    s_runtime_frame_size = CAMERA_FRAME_SIZE;
    s_buffer_frame_size = buffer_frame_size;
...
{
    const int runtime_override = s_runtime_fps_override;
    ...
    if (link_age_us < 15LL * 1000000LL) {
        ...
        upload_fps = camera_stream_get_target_fps();
    }
#else
    if (CONFIG_VIDEO_UDP_WARMUP_STEP_SECONDS == 0) {
        return camera_stream_get_target_fps();
    }
...
#endif
    const int target_fps = camera_stream_get_target_fps();
    return upload_fps < target_fps ? upload_fps : target_fps;
}
...
static void camera_udp_task(void *argument)
{
    ...
    while (true) {
        ...
        if (uplink_generation != active_uplink_generation) {
            ...
            ESP_LOGI(TAG, "Video UDP socket reopen requested for uplink generation %lu",
        ...
        const int selected_upload_fps = cloud_ready
            ? upload_fps_for_link_age(link_ready_us) : camera_stream_get_target_fps();
        ...
        if (xSemaphoreTake(s_camera_control_lock, portMAX_DELAY) != pdTRUE) {
...
esp_err_t camera_stream_start(void)
{
    ...
    if (s_camera_control_lock == NULL) {
        s_camera_control_lock = xSemaphoreCreateMutex();
        ESP_RETURN_ON_FALSE(s_camera_control_lock != NULL, ESP_ERR_NO_MEM, TAG,
                            "Failed to create camera control lock");
    }
