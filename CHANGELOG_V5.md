...
本次只对准备推送的仓库副本做脱敏，不修改本地工程：

- 真实公网 IP、推流/API/WebSocket 地址替换为 RFC 5737 TEST-NET 示例 `192.0.2.10`。
- AP、Web 管理和演示登录口令替换为 `CHANGE_ME_*` 占位符。
- 本机工程路径替换为通用示例路径。
- 地图默认坐标替换为 `0,0`。
- `sdkconfig`、构建目录、日志、抓包和带现场配置的预编译固件不提交。

这些占位值不能直接用于部署。部署前只在私有副本或构建环境中填入实际地址和口令，不要把现场配置再次提交到公共仓库。

## 已知安全边界

当前通信协议保持已验证逻辑不变，没有因本次脱敏新增认证协议。公网部署时必须使用防火墙、安全组、VPN/专线和 HTTPS/WSS 限制访问；演示登录占位逻辑不能当作生产鉴权。机器人最终失联停车必须由 STM32 本地 500 ms 看门狗保证。
