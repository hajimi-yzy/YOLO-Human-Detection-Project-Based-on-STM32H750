...
private:
	...
	uint16_t angle_set;  //设置的舵机角度
	uint16_t last_angle; //上一次的角度，用于避免出现NaN
...
public:
	...
	void set_real_angle(uint16_t angle); //设置实际角度
	uint16_t read_real_angle();
};
...
uint8_t check_sum(uint8_t *send_data); //校验和
