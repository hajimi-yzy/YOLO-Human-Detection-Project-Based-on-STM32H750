...
{
	if(isnan(angle))
		return; //若输入的浮点数为NaN则不设置舵机角度，避免炸鸡
	else
		this->angle_set = angle / PI * 750 + 500; // 将弧度转换为舵机值
}
...
// 计算校验和
uint8_t check_sum(uint8_t *send_data)
{
	static uint16_t checksum;
	uint8_t data_len = send_data[3];
...
// 计算校验和
uint8_t check_sum(volatile uint8_t *send_data)
{
	static uint16_t checksum;
	uint8_t data_len = send_data[3];
	checksum = send_data[2] + send_data[3] + send_data[4];
...
void Servo::move(uint8_t *send_buffer)
{
	...
	send_buffer[5] = this->angle_set;
	send_buffer[6] = this->angle_set >> 8;
	send_buffer[7] = this->move_time;
	send_buffer[8] = this->move_time >> 8;
	send_buffer[9] = check_sum(send_buffer);
}
...
void Servo::read_angle(uint8_t *send_buffer)
{
	...
	send_buffer[1] = 0x55;
	send_buffer[2] = this->id;
	send_buffer[3] = SERVO_POS_READ_LEN;
	send_buffer[4] = SERVO_POS_READ;
	send_buffer[5] = check_sum(send_buffer);
}

void Servo::set_real_angle(uint16_t angle)
{
	this->angle = angle;
}
...
void Servo_Broad_Cast::unload(uint8_t *send_buffer)
{
	...
	send_buffer[6] = check_sum(send_buffer);
}
