...
Position3 operator-(const Position3 &pos1, const Position3 &pos2);
Position3 operator*(const Position3 &pos1, const float k);
...
class user_single_key
{
private:
    uint32_t key_index;
public:
    user_single_key(uint32_t key_index);
    user_single_key(){};
...
{
private:
    bool key_flags[100]; //标志是否刚按下按键，0表示未按下
    uint32_t key_count;
public:
    user_single_key key_init();
    bool if_single_press(user_single_key ukey,uint8_t key);
};

bool value_limit(float &val, float min, float max);

float loop_constrain(float Input, float minValue, float maxValue);// 循环限幅

extern Single_Press single_Press;
