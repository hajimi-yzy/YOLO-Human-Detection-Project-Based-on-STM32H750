...
{
    // 计算机械腿相对于起始端的末端坐标
    Pws[0] = fkine(Thetas(LEG1_THETA_STAND_1, THETA_STAND_2, THETA_STAND_3));
    Pws[1] = fkine(Thetas(LEG2_THETA_STAND_1, THETA_STAND_2, THETA_STAND_3));
    Pws[2] = fkine(Thetas(LEG3_THETA_STAND_1, THETA_STAND_2, THETA_STAND_3));
    Pws[3] = fkine(Thetas(LEG4_THETA_STAND_1, THETA_STAND_2, THETA_STAND_3));
    Pws[4] = fkine(Thetas(LEG5_THETA_STAND_1, THETA_STAND_2, THETA_STAND_3));
    Pws[5] = fkine(Thetas(LEG6_THETA_STAND_1, THETA_STAND_2, THETA_STAND_3));
    // 默认站立坐标，这里copy一份
...
static Thetas ikine(Position3 &pos)
{
    ...
    pos1 = pos;
    arm_sqrt_f32(pow(pos1.x, 2) + pow(pos1.y, 2),&R);
    arm_sqrt_f32(pow(pos1.z, 2) + pow((R - LEG_LEN1), 2),&Lr);
...
{
    Position3 retvel;
    static float cos_rotate_angle_x,cos_rotate_angle_y,cos_rotate_angle_z,
                 sin_rotate_angle_x,sin_rotate_angle_y,sin_rotate_angle_z;
    static uint32_t last_LegControl_round;
    if(LegControl_round!=last_LegControl_round)  //减少重复运算
    {
        cos_rotate_angle_x = arm_cos_f32(rotate_angle.x);
        cos_rotate_angle_y = arm_cos_f32(rotate_angle.y); 
        cos_rotate_angle_z = arm_cos_f32(rotate_angle.z); 
        sin_rotate_angle_x = arm_sin_f32(rotate_angle.x);
        sin_rotate_angle_y = arm_sin_f32(rotate_angle.y);
        sin_rotate_angle_z = arm_sin_f32(rotate_angle.z);
    }
    last_LegControl_round = LegControl_round;

    retvel.x = cos_rotate_angle_y * cos_rotate_angle_z * (P_legs[index].x + point.x) - cos_rotate_angle_y * sin_rotate_angle_z * (P_legs[index].y + point.y);
    retvel.y = (cos_rotate_angle_x * sin_rotate_angle_z + cos_rotate_angle_z * sin_rotate_angle_x * sin_rotate_angle_y) * (P_legs[index].x + point.x) + (cos_rotate_angle_x * cos_rotate_angle_z - sin_rotate_angle_x * sin_rotate_angle_y * sin_rotate_angle_z) * (P_legs[index].y + point.y);
    retvel.z = point.z + (sin_rotate_angle_x * sin_rotate_angle_z - cos_rotate_angle_x * cos_rotate_angle_z * sin_rotate_angle_y) * (P_legs[index].x + point.x) + (cos_rotate_angle_z * sin_rotate_angle_x + cos_rotate_angle_x * sin_rotate_angle_y * sin_rotate_angle_z) * (P_legs[index].y + point.y);
...
void Gait_prg::CEN_and_pace_cal()
{
    ...
    if (velocity_s.Vx >= 0)
        ...
        CEN.x = -sqrt(pow(module_CEN, 2) / (1 + pow(velocity.Vx, 2) / pow(velocity.Vy, 2)));
    // 计算步伐大小
    module_speed = pow(pow(velocity.Vx, 2) + pow(velocity.Vy, 2) + pow(velocity.omega, 2), 1.0f / 2);
    ...
    if (R_pace > MAX_R_PACE)
        this->pace_time = PACE_TIME_DEFALUT / (R_pace / MAX_R_PACE); // 若超过最大步伐大小则缩小步伐时间
    else
        this->pace_time = PACE_TIME_DEFALUT; // 若小于最大步伐大小则固定步伐时间
    if (R_pace > MAX_R_PACE)
...
void Gait_prg::gait_proggraming()
{
    ...
    float sp_phase_ratio=0; //支撑相占比(support phase ratio)
    uint32_t temp_round = 0;
    for (int i = 0; i < 6; i++)
    {
        //先确定每条腿的回合数
        switch (gait_mode)
        {
        case TRIPOD: //三角步态
            sp_phase_ratio = 0.5;   //1./2
            if (i == 0 || i == 2 || i == 4)
                temp_round = LegControl_round;
            else
                temp_round = (LegControl_round + N_POINTS / 2) % N_POINTS;
            break;
        case QUADPOD: //四角步态
            sp_phase_ratio = 2./3;   //2./3
            if (i == 0 || i == 4)       temp_round =  LegControl_round;
            else if (i == 1 || i == 5)  temp_round = (LegControl_round + N_POINTS / 3) % N_POINTS;
            else if (i == 2 || i == 3)  temp_round = (LegControl_round + 2 * N_POINTS / 3) % N_POINTS;
            break;
        case PENTAPOD:  //五角步态
            sp_phase_ratio = 5./6;  //5./6
            if (i == 0 || i == 1 || i == 2)
                temp_round = (LegControl_round + N_POINTS / 6 * 2 * i) % N_POINTS;
            else
                temp_round = (LegControl_round + N_POINTS / 6 * (2 * i - 5)) % N_POINTS;
            break;
        default:
            break;
        }
        if (temp_round <= N_POINTS * sp_phase_ratio) // 画下半圆
        {
            angle_t = angle_off[i] + d_theta / 2 - step_size * (1 / (2 * sp_phase_ratio)) * temp_round;  // 计算这个点的角度
        ...
        {
            angle_t = angle_off[i] - d_theta / 2 + step_size * (1 / (2 * (1 - sp_phase_ratio))) * (temp_round - N_POINTS * sp_phase_ratio); // 计算这个点的角度
            point.x = Vec_Leg_Start2CEN_s[i].x + norm_CEN2legs[i] * cos(angle_t);                 // 计算这个点的x轴坐标(相对于机械腿起始端)
            point.y = Vec_Leg_Start2CEN_s[i].y + norm_CEN2legs[i] * sin(angle_t);                 // 计算这个点的y轴坐标(相对于机械腿起始端)
            y_temp = (double)(temp_round - N_POINTS * sp_phase_ratio) / (double)(N_POINTS * (1 - sp_phase_ratio)) * PI;
            point.z = R_pace*sin(y_temp) * Kz * Rp_ratios[i] + Pws[i].z;
        }
